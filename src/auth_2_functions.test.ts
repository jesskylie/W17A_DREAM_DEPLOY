...
import { adminAuthRegister, adminAuthLogin, adminUserDetails, updatePassword } from './auth';
...
describe('Testing update password', () => {
    test('Old password does not match old password', () => {
        const newUser = adminAuthRegister('jake@hotmail.com', 'password1234', 'Jake', 'Garry') as TokenString;
        const result = updatePassword(newUser.token, 'password123456', 'helloworld123');
        expect(result).toStrictEqual({ error: expect.any(String), errorCode: 401 });
    });
    
    test('New password and old password match exactly', () => {
        const newUser = adminAuthRegister('jake@hotmail.com', 'password1234', 'Jake', 'Garry') as TokenString;
        const result = updatePassword(newUser.token, 'password1234', 'password1234');
        expect(result).toStrictEqual({ error: expect.any(String), errorCode: 401 });
    });
    ...
    test('Successfully updated new password', () => {
        const newUser = adminAuthRegister('jake@hotmail.com', 'password1234', 'Jake', 'Garry') as TokenString;
        const result = updatePassword(newUser.token, 'hello1234567', 'password1234');
        expect(result).toStrictEqual({});
    });
    
    test('Token is invalid ', () => {
        adminAuthRegister('jake@hotmail.com', 'password1234', 'Jake', 'Garry') as TokenString;
