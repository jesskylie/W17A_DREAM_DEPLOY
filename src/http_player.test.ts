...
describe('Test: GET /v1/player/{playerid}/results', () => {
  test('Success - valid input', () => {
    ...
    const questionId = requestCreateQuestionV2(
      result.body.token,
      question,
      quizId.quizId
    );
      ...
      questionResults: [
        {
          ...
          playersCorrectList: [expect.any(String), expect.any(String)],
  ...
  test('Session is not in FINAL_RESULTS state - valid input', () => {
    ...
    const playerId1 = requestPlayerCreate(sessionId.sessionId, '');
    expect(() => requestSessionFinalResult(playerId1.playerId)).toThrow(
      HTTPError[400]
    );
  });
});
