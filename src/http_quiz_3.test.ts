...
describe('Testing POST /v2/admin/quiz', () => {
  ...
  test('Name contains invalid characters - error 400 ', () => {
    ...
    expect(() =>
      requestAdminQuizCreateV2(result.body.token, '$$$$', 'Quiz description')
  ...
  test('Name is less than not between 3 - 30 characters long - error 400 ', () => {
    ...
    expect(() =>
      requestAdminQuizCreateV2(result.body.token, 'A', 'Quiz description')
    ).toThrow(HTTPError[400]);
    expect(() =>
      requestAdminQuizCreateV2(
        result.body.token,
  ...
  test('Name is already used by the current logged in user for another quiz ', () => {
    ...
    expect(() =>
      requestAdminQuizCreateV2(result.body.token, 'QuizOne', 'description')
    ).toThrow(HTTPError[400]);
  });
...
describe('Testing DELETE /v2/admin/quiz', () => {
  test('Send a quiz to trash', () => {
    ...
    expect(
      requestAdminQuizRemoveV2(result.body.token, quizId.quizId)
    ).toStrictEqual({});
  });
  ...
  test('Token is empty or invalid - error 401', () => {
    ...
    expect(() => requestAdminQuizRemoveV2('', quizId.quizId)).toThrow(
      HTTPError[401]
    );
    expect(() => requestAdminQuizRemoveV2('abcde', quizId.quizId)).toThrow(
      HTTPError[401]
    );
  });
  ...
  test('Valid token is provided but user is not owner of quiz - error 403', () => {
    ...
    expect(() =>
      requestAdminQuizRemoveV2(userTwo.body.token, quizId.quizId)
...
describe('Testing GET /v2/admin/quiz/:quizid', () => {
  ...
  test('Token is empty/invalid - 400 error', () => {
    ...
    expect(() => requestAdminQuizInfoV2('', quizId.quizId)).toThrow(
      HTTPError[401]
    );
    expect(() => requestAdminQuizInfoV2('abcdef', quizId.quizId)).toThrow(
      HTTPError[401]
    );
  });
  test('Valid token is provided but user is not owner of quiz - error 403', () => {
    ...
    requestAdminQuizCreateV2(userOne.body.token, 'QuizTwo', 'Quiz description');
    expect(() =>
      requestAdminQuizInfoV2(userTwo.body.token, quizId.quizId)
    ).toThrow(HTTPError[403]);
  });
});
...
describe('Testing PUT /v2/admin/quiz/:quizid/name', () => {
  test('Success name update', () => {
    ...
    expect(
      requestUpdateQuizNameV2(userOne.body.token, quizId.quizId, 'New Name')
    ).toStrictEqual({});
  });
  test('Testing name contains invalid characters - 400 error', () => {
    ...
    expect(() =>
      requestUpdateQuizNameV2(userOne.body.token, quizId.quizId, '$$$$$$$e')
    ).toThrow(HTTPError[400]);
  });
  test('Testing name not between 3 and 30 characters - 400 error', () => {
    ...
    expect(() =>
      requestUpdateQuizNameV2(userOne.body.token, quizId.quizId, 'a')
    ).toThrow(HTTPError[400]);
    expect(() =>
      requestUpdateQuizNameV2(
        userOne.body.token,
  ...
  test('Testing name is already used for another quiz - 400 error', () => {
    ...
    expect(() =>
      requestUpdateQuizNameV2(userOne.body.token, quizId.quizId, 'QuizOne')
    ).toThrow(HTTPError[400]);
  });
  ...
  test('Token is empty/invalid - 400 error', () => {
    ...
    expect(() =>
      requestUpdateQuizNameV2('', quizId.quizId, 'New Quiz Name')
    ).toThrow(HTTPError[401]);
    expect(() =>
      requestUpdateQuizNameV2('abcdef', quizId.quizId, 'New Quiz Name')
    ).toThrow(HTTPError[401]);
  });
  test('Valid token is provided but user is not owner of quiz - error 403', () => {
    requestClear();
...
describe('Testing PUT /v2/admin/quiz/:quizid/description', () => {
  ...
  test('Description is more than 100 characters - 400 error', () => {
    ...
    expect(() =>
      requestAdminQuizDescriptionUpdateV2(
        userOne.body.token,
        quizId.quizId,
        '1234567891011121314151617181920212324256272829303132333435636839465051525354556575859606162636465667689710'
      )
    ).toThrow(HTTPError[400]);
  });
  test('Token is empty/invalid - 401 error', () => {
    ...
    expect(() =>
      requestAdminQuizDescriptionUpdateV2('', quizId.quizId, 'New Quiz Name')
    ).toThrow(HTTPError[401]);
    expect(() =>
      requestAdminQuizDescriptionUpdateV2(
        'abcdef',
